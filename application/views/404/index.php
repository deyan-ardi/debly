<!-- main body - start
			================================================== -->
<main>


    <!-- error_section - start
				================================================== -->
    <section id="error_section"
        class="error_section bg_gradient_blue deco_wrap d-flex align-items-center text-white clearfix">
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-lg-8">
                    <div class="error_content text-center">
                        <h1 class="page_title">404</h1>
                        <h3 class="title_text">Stop! Anda Kesasar</h3>
                        <p>
                            Sepertinya tautan yang Anda masukkan tidak sesuai dengan resource apapun yang ada pada
                            website ini, coba kembali ke Beranda. Jika Anda yakin ini kesalahan, silahkan hubungi
                            administrator server
                        </p>
                        <a href="<?= base_url() ?>" class="btn bg_white">Ke Beranda</a>
                    </div>
                </div>
            </div>
        </div>

        <div class="deco_image spahe_1" data-aos="fade-down" data-aos-delay="100">
            <img src="<?= base_url() ?>assets/images/shapes/shape_1.png" alt="spahe_not_found">
        </div>
        <div class="deco_image spahe_2" data-aos="fade-up" data-aos-delay="300">
            <img src="<?= base_url() ?>assets/images/shapes/shape_2.png" alt="spahe_not_found">
        </div>
    </section>
    <!-- error_section - end
				================================================== -->


</main>
<!-- main body - end
			================================================== -->