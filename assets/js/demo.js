$().ready(function() {
    ...
    $('.fixed-plugin .background-color span').click(function() {
        ...
        if ($sidebar.length != 0) {
            ...
            $profil.attr('class', new_color);
            $btn_bg.attr('class', new_color);
        }
    ...
    $($btn_singkat).click(function () {
        if ($('#url_old').val() != "") {
            $.ajax({                    
                url: 'result_tautan',     
                type: "POST", // performing a POST request
                data : {
                    data1: $('#url_old').val(), // will be accessible in $_POST['data1']
                },
                dataType: 'text',                   
                success: function(data)         
                {
                    $($btn_result).append(data);
...
demo = {
    ...
    showNotification: function(from, align) {
        ...
        $.notify({
            icon: "nc-icon nc-app",
            message: "Selamat Datang di <b>Deb.ly Dashboard</b> - Selamat Menggunakan Sistem Ini, Jaga Selalu Data Anda."
...
const kirim = () => {
    const result = document.getElementById('alert-information');
    const data = new FormData()
    let value = document.getElementById('url_custom').value;
    data.append("url_custom", value);
    if (value != "") {
        fetch(baseURL+"user/kustom_tautan", {
            method: "POST",
            body: data,
        })
        ...
        .then(data => {
            ...
            if (data != "") {
                document.getElementById("kustom").setAttribute("disabled", "true");
                result.classList.add("mt-3");
            } else {
                document.getElementById("kustom").removeAttribute("disabled");
                 result.classList.remove("mt-3");
            }
        });
    }
}
...
const copyNew = () => {
    document.getElementById('data-url-new').select();
